...
class PROBINGEvalWithKey(PROBINGEval):
    def __init__(self, task, task_path, seed=1111):
        super(PROBINGEvalWithKey,self).__init__(task, task_path, seed)
    ...
    def run(self, params, batcher):
        ...
        for key in self.task_data:
            ...
            task_embed[key]['X'] = []
            for ii in range(0, len(self.task_data[key]['y']), bsize):
                batch_with_key = list(zip(self.task_data[key]['X'][ii:ii + bsize]\
                                          ,self.task_data[key]['y'][ii:ii + bsize]))
...
class WordContentWithV(PROBINGEval):
    def __init__(self, task_path, seed=1111):
        task_path = os.path.join(task_path, 'word_content_addw.txt')
        # labels: 200 target words
        PROBINGEval.__init__(self, 'WordContent', task_path, seed)
"""
